...
//            return user.getUsername() + "님은 이미 회원이십니다";
    ...
    public String shelterGetList(Model model, @PageableDefault(size=2, sort="no",
    ...
    public String adminShelterSearch(@RequestParam(value = "searchArea", required = false) String searchArea,
                                     ...
                                     Model model, @PageableDefault(size=2, sort="no",
            direction = Sort.Direction.DESC) Pageable pageable) {
    ...
    public String u_shelterGetList(Model model, @PageableDefault(size=2, sort="no",
            direction = Sort.Direction.DESC) Pageable pageable){
        int currentPage = pageable.getPageNumber() + 1;
        int blockSize = 2;
        int curBlock = (currentPage -1) / blockSize;
